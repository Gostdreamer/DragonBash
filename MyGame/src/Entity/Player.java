...
public class Player extends MapObject
{
	...
	public Player(TileMap tm)
	{
		...
		try
		{
			...
			// This line WILL NOT WORK unless you have specified the resources
			// folder as a project path
			...
			for (int i = 0; i < 7; i++)
			{
				...
				for (int j = 0; j < numFrames[i]; j++)
				{
					...
					// if it is scratching, double the width and height (larger
					// sprite)
					else
	...
	public void update()
	{
		...
		// if you are walking or idle, and you are not changing elevation, reset
		// the double jumps
	...
	public void checkAttack(ArrayList<Enemy> enemies)
	{
		...
		for (int i = 0; i < enemies.size(); i++)
		{
			...
			if (scratching)
			{
				...
				else if (e.getX() < x && e.getX() > x - scratchRange && e.getY() > y - height / 2 && e.getY() < y + height / 2)
					e.hit(scratchDamage);
			}
